...
def dropCorrelatedFeatures(df,correlationLimit):
    corr_matrix = df.corr().abs()
...
def listNA(df):
    print(df.isna().apply(pd.Series.value_counts, normalize=True))
...
def removeNACols(df):
    return df.dropna(axis=1, thresh=(len(df)/10), how="any") 
